...
| | |
|---|---|
| [P2Flux/sdk-js](https://github.com/P2Flux/sdk-js) | TypeScript/JavaScript, zero dependencies |
| [P2Flux/sdk-php](https://github.com/P2Flux/sdk-php) | PHP 8.1+, curl by default — for WordPress/WooCommerce and Laravel hosts |
...
Install from the repository until it is on Packagist, pinned to a tag:
...
Nothing here is language-specific — a client is an HTTP call plus this result table. The reference
behaviour is [`src/index.ts`](https://github.com/P2Flux/sdk-js/blob/main/src/index.ts) in the JS
SDK.
...
It authorizes charging **that one subscription, on the terms the customer signed** — it cannot
change the amount, the period or the recipient, and it cannot move funds anywhere else. That is the
bound worth understanding: a leaked reference lets someone trigger a payment the customer already
authorized, to the merchant the customer already chose.
